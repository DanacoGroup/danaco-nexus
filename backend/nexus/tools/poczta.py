...
def _client(ctx: ToolContext) -> MailClient:
    try:
        config = load_config(ctx.settings)
...
@registry.register(
    "mail_list",
    """Najnowsze wiadomości e-mail użytkownika z folderu (domyślnie Odebrane) oraz lista folderów
konta. Zwraca UID, nadawcę, temat, datę i flagi – treść czyta mail_read.""",
    MailListInput,
)
def mail_list(ctx: ToolContext, args: MailListInput) -> ToolResult:
    ...
    return ToolResult(
        ...
        f"Poczta {args.folder}: {len(listing['messages'])} z {listing['total']} wiadomości",
    )
...
def mail_search(ctx: ToolContext, args: MailSearchInput) -> ToolResult:
    try:
        ...
        raise ToolError(str(error)) from error
    return ToolResult(
        {"folder": args.folder, "total": found["total"], "messages": _short(found["messages"])},
...
def mail_send(ctx: ToolContext, args: MailComposeInput) -> ToolResult:
    try:
        ...
        raise ToolError(str(error)) from error
    attachments_for(ctx, data["file_ids"])
    summary = f"Do: {', '.join(data['to'])} – {data['subject']}"
