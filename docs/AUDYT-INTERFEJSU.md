...
| Sprawa | Stan |
|---|---|
| `design-system/Apps/` — kopie SMS i wykazów połączeń (30 MB, 11 plików) | do decyzji właściciela: usunąć albo przenieść poza repozytorium; katalog jest pominięty w `.gitignore`, ale **trafia do kopii zapasowych** (`materialy.tar.zst` obejmuje `design-system`, sprawdzone 24.09.2026) |
| Pakiety marki poza kontrolą wersji (1,9 GB: `promocja` 708 MB, `branding` 593 MB, `landing` 395 MB, `motion` 112 MB, pozostałe) | katalogi są nieśledzone; **kopia zapasowa je obejmuje** (`deploy/kopia-zapasowa.sh`, `materialy.tar.zst`, 14 dni). Otwarte: kopie leżą na tym samym serwerze (`dane/kopie`) — awaria dysku zabiera oryginał i kopię naraz |
