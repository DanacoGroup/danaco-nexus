...
| Połączenie z Nexusem | Po zalogowaniu w oknie mostek JS tworzy klucz urządzenia `POST /api/urzadzenia {kind: "android"}` i przekazuje go do magazynu natywnego (EncryptedSharedPreferences). Moduły natywne używają `Authorization: Bearer nxd_…`. Cofnięcie klucza na stronie „Urządzenia” kończy też sesję okna, z której klucz powstał — aplikacja usuwa klucz, a nowy tworzy dopiero po ponownym zalogowaniu w oknie aplikacji (klucze sprzed 24.09.2026 nie mają powiązanej sesji: tworzone są ponownie po wejściu do aplikacji). | automatycznie |
...
(dziś `versionCode 2`, `versionName "1.0.0"`). Telefon odrzuca instalację aktualizacji
o tym samym albo niższym `versionCode`, a komunikat systemu nie mówi o tym wprost —
wygląda to jak zepsuta paczka.

Numer produktu jest od 21 września 2026 wspólny dla wszystkich składników: **1.0.0**
w `backend/nexus/__init__.py`, `backend/pyproject.toml`, `frontend/package.json`,
`desktop/package.json`, `extension/manifest.json` i tutaj. `versionCode` biegnie osobno,
bo Android wymaga liczby rosnącej przy każdej paczce.
